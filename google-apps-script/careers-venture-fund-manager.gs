/**
 * Persist Foundry — Careers: Venture Fund Manager → Google Sheet
 *
 * Spreadsheet:
 * https://docs.google.com/spreadsheets/d/1KHamhHBBwHP1hIHjq_OoaUNHRy_tF-frGwxr9Q7mWzc/
 *
 ...
 *      - Description: Careers Venture Fund Manager
 *      - Execute as: Me
 ...
 *      {"success":true,"service":"Persist Foundry Careers — Venture Fund Manager",...}
 ...
 *      VITE_CAREERS_VFM_SCRIPT_URL=https://script.google.com/macros/s/XXXX/exec
...
var SHEET_ID = '1KHamhHBBwHP1hIHjq_OoaUNHRy_tF-frGwxr9Q7mWzc';
var HEADERS = [
  'Full Name',
  'Email',
  'LinkedIn',
...
function doGet(e) {
  ...
  return respond({
    success: true,
    service: 'Persist Foundry Careers — Venture Fund Manager',
...
function handleApplication(data) {
  ...
  sheet.appendRow([
    ...
    String(
      data.roleTitle || data['role-title'] || 'Venture Fund Manager',
    ).trim(),
    String(
      data.formId || data['form-id'] || 'careers-page-Venture-Fund-Manager',
    ).trim(),
  ...
  Logger.log(
    JSON.stringify(
      handleApplication({
        ...
        roleTitle: 'Venture Fund Manager',
        formId: 'careers-page-Venture-Fund-Manager',
      }),
    ),
  );
}
