/**
 * Persist Foundry — Careers: Startup Founder → Google Sheet
 *
 * Spreadsheet:
 * https://docs.google.com/spreadsheets/d/1tUyhuJ9VRw6pZFrkbEy5IWCHHCkzwEVDMXwEBS7MuYw/
 *
 ...
 *      - Description: Careers Startup Founder
 *      - Execute as: Me
 ...
 *      {"success":true,"service":"Persist Foundry Careers — Startup Founder",...}
 ...
 *      VITE_CAREERS_STARTUP_FOUNDER_SCRIPT_URL=https://script.google.com/macros/s/XXXX/exec
...
var SHEET_ID = '1tUyhuJ9VRw6pZFrkbEy5IWCHHCkzwEVDMXwEBS7MuYw';
var HEADERS = [
  'Full Name',
  'Email',
  'LinkedIn',
...
function doGet(e) {
  ...
  return respond({
    success: true,
    service: 'Persist Foundry Careers — Startup Founder',
...
function handleApplication(data) {
  ...
  sheet.appendRow([
    ...
    String(data.roleTitle || data['role-title'] || 'Startup Founder').trim(),
    String(data.formId || data['form-id'] || 'startup-founder').trim(),
...
function testAppend() {
  Logger.log(
    JSON.stringify(
      handleApplication({
        formType: 'Careers-Startup-Founder',
